...
async def _async_index_repository(repo_id: uuid.UUID, user_id: uuid.UUID) -> bool:
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            logger.info(f"Upserting vectors into Qdrant...")
...
def index_repository_task(self, repo_id_str: str, user_id_str: str) -> bool:
    ...
    # Run async pipeline synchronously in Celery worker thread
    loop = asyncio.get_event_loop()
    if loop.is_running():
        # If loop is already running (e.g. inside a container environment)
        future = asyncio.run_coroutine_threadsafe(_async_index_repository(repo_id, user_id), loop)
        return future.result()
    else:
        return loop.run_until_complete(_async_index_repository(repo_id, user_id))
...
def process_document_task(self, doc_id_str: str) -> bool:
    ...
    loop = asyncio.get_event_loop()
    if loop.is_running():
        future = asyncio.run_coroutine_threadsafe(_async_process_document(doc_id), loop)
        return future.result()
    else:
        return loop.run_until_complete(_async_process_document(doc_id))
